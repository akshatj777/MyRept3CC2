Feature: Patient status in Post Acute Onboarding Worklist
  ...
  Scenario Outline: Patient status in post onboarding worklist when Add today a Non BPCI DRG, working - Patient's episode is activated, patient will now show in the worklist.
     Given I am on the login page
